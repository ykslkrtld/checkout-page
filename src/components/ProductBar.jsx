...
const ProductBar = ({getProducts}) => {
    ...
    const postProduct = async (newProducts) => {
        try {
            await axios.post("https://6615610db8b8e32ffc7ac40a.mockapi.io/products/", newProducts )
        } catch (error) {
            console.log(error)
        }
        getProducts()
  ...
  return (
    ...
    <Form.Label htmlFor="basic-url">Product Image</Form.Label>
      <InputGroup className="mb-3">
        ...
        <Form.Control id="basic-url" aria-describedby="basic-addon3" onChange={(e)=> setImage(e.target.value)} value={image}/>
      </InputGroup>
